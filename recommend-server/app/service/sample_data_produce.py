...
cocktail_features = np.random.rand(num_cocktails, 5)
...
for user_id, cocktail_id in zip(user_ids, cocktail_ids):
    ...
    cocktail_feat = cocktail_features[cocktail_id]
...
cocktail_features_df = pd.DataFrame(cocktail_features, columns=['sweet', 'sour', 'bitter', 'alcohol', 'carbonation'])
cocktail_features_df.index.name = 'cocktail_id'
...
cocktail_features_df.to_csv(save_dir + 'cocktail_features.csv')
