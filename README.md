...
네비게이션 바마이브러리, 메인피드페이지, PP(채팅)페이지, 검색페이지로 이동 가능


댓글

텍스트를 입력하고 5가지의 댓글 색을 선택해 댓글 작성 가능
대댓글 수를 클릭해 대댓글 조회, 답글달기를 클릭해 대댓글 작성 가능
...
1컷 레이아웃 18개, 2컷 레이아웃 19개, 총 37개의 레이아웃으로 다양한 스레드 작성 가능

이미지 컷 수 뿐만 이미지와 텍스트 칸의 비율도 다양함
...
카테고리 추가 : 새 카테고리 추가
카테고리 수정 : 카테고리를 드래그해 순서 조정 ( 드래그 후 즉시 반영으로 사용자 친화적 UI )
카테고리 이름 수정 가능
...
책의 페이퍼가 넘어가는 UI를 적용해 아날로그적 감성을 느낄 수 있는 책 페이지
좋아요, 댓글 기능을 제외해 책 자체의 내용에 집중할 수 있도록 함
페이퍼 옵션 : 해당 페이퍼가 포함된 스레드로 이동하기 -> 스레드 단건 조회
해당 페이퍼를 책에서 제거하기
...
인기검색어 : 엘라스틱 서치를 통해 최근 사용자들에게 가장 많이 검색된 검색어 조회 가능
